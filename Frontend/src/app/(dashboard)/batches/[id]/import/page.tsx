        ...
        <div className="card p-5 space-y-4">
          ...
          {result.failed === 0 ? (
            <div className="flex items-center gap-2 text-green-600 text-sm">
              <CheckCircle size={16} />
              {result.saved} new question(s) saved{result.duplicates > 0 ? `, ${result.duplicates} skipped as duplicates` : ''}. Batch advanced to validation_complete.
            </div>
          ) : (
          ...
          {result.duplicates > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-amber-600">Skipped as duplicates (already exist in this chapter)</p>
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {result.results.filter((r) => r.duplicate).map((r) => (
                  <div key={r.index} className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700 flex items-center justify-between">
                    <span>Question #{r.index + 1}</span>
                    <span>{r.similarity === 1 ? 'exact match' : `${Math.round((r.similarity ?? 0) * 100)}% similar`}</span>
                  </div>
                ))}
              </div>
            </div>
